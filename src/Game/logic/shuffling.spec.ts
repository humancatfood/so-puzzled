...
describe('Shuffling', () => {
  ...
  describe.each(stages)('stage: %j', stage => {
    ...
    describe('basic', () => {
      it('shuffles', () => {
        expect(shufflePiece(stage)).toEqual(expect.objectContaining({ 
      ...
      it('doesn\'t repeat itself .. EVER!!!!', () => {
        const locations = (new Array(100)).fill(undefined).map(() => shufflePiece(stage))
      ...
      it('stays within limits .. ALWAYS!!!!', () => {
        ...
        const locations = (new Array(300)).fill(undefined).map(() => shufflePiece(stage))
    ...
    describe('obstacle avoidance', () => {
      it('shuffles around things', () => {
        ...
        const locations = (new Array(500)).fill(undefined).map(() => shufflePiece(stage, [obstacle]))
        const locationsInObstacle = locations.filter(({ left, top }) => left <= obstacle.right && left >= obstacle.left && top >= obstacle.top && top <= obstacle.bottom)
